...
void main() {
  runApp(EightLetterQuiz());
}

class EightLetterQuiz extends StatefulWidget {
  EightLetterQuiz({super.key});

  @override
  State<EightLetterQuiz> createState() => _EightLetterQuizState();
}

class _EightLetterQuizState extends State<EightLetterQuiz> {

  int i = 0;
  final TextEditingController _eightletterController = TextEditingController();
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => EightLetterQuiz()));
                  },
                ),
              ],
            );
  ...
  void checkWord() {
    if (_eightletterController.text == '') {
      return;
    }
    ...
    setState(() {
      if (uniqueWords.contains(_eightletterController.text.toLowerCase())) {
        if (!correctGuesses
            .contains(_eightletterController.text.toUpperCase())) {
          correctGuesses.add(_eightletterController.text.toUpperCase());
        }
        ...
        if (!incorrectGuesses
            .contains(_eightletterController.text.toUpperCase())) {
          incorrectGuesses.add(_eightletterController.text.toUpperCase());
        }
      }
    });

    _eightletterController.clear();
  }
  ...
  void initState() {
    super.initState();
    getWords();
    _eightletterController.addListener(() {
      final String text = _eightletterController.text.toUpperCase();
      _eightletterController.value = _eightletterController.value.copyWith(
        text: text,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('8 letter words',
            style: TextStyle(
              fontSize: 20,
            )),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  child: TextField(
                    controller: _eightletterController,
