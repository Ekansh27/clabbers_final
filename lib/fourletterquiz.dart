...
class FourLetterQuiz extends StatefulWidget {
  FourLetterQuiz({super.key});
  @override
  State<FourLetterQuiz> createState() => _FourLetterQuizState();
}

class _FourLetterQuizState extends State<FourLetterQuiz> {
  int i = 0;
  final TextEditingController _fourletterController = TextEditingController();
  ...
  void getWords() async {
    ...
    final x = await rootBundle.loadString("assets/csvfile/fours.csv");
    setState(() {
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => FourLetterQuiz()));
                  },
                ),
              ],
            );
  ...
  void checkWord() {
    ...
    setState(() {
      if (uniqueWords.contains(_fourletterController.text.toLowerCase())) {
        if (!correctGuesses
            .contains(_fourletterController.text.toUpperCase())) {
          correctGuesses.add(_fourletterController.text.toUpperCase());
        }
        ...
        if (!incorrectGuesses
            .contains(_fourletterController.text.toUpperCase())) {
          incorrectGuesses.add(_fourletterController.text.toUpperCase());
        }
      }
    });

    _fourletterController.clear();
  }
  ...
  void initState() {
    super.initState();
    getWords();
    _fourletterController.addListener(() {
      final String text = _fourletterController.text.toUpperCase();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('4 letter words',
            style: TextStyle(
              fontSize: 20,
            )),
