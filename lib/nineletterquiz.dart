...
class NineLetterQuiz extends StatefulWidget {
  NineLetterQuiz({super.key});
  @override
  State<NineLetterQuiz> createState() => _NineLetterQuizState();
}

class _NineLetterQuizState extends State<NineLetterQuiz> {
  int i = 0;
  final TextEditingController _nineletterController = TextEditingController();
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => NineLetterQuiz()));
                  },
                ),
              ],
            );
  ...
  void checkWord() {
    ...
    setState(() {
      if (uniqueWords.contains(_nineletterController.text.toLowerCase())) {
        if (!correctGuesses
            .contains(_nineletterController.text.toUpperCase())) {
          correctGuesses.add(_nineletterController.text.toUpperCase());
        }
        ...
        if (!incorrectGuesses
            .contains(_nineletterController.text.toUpperCase())) {
          incorrectGuesses.add(_nineletterController.text.toUpperCase());
        }
      }
    });

    _nineletterController.clear();
  }
  ...
  void initState() {
    super.initState();
    getWords();
    _nineletterController.addListener(() {
      final String text = _nineletterController.text.toUpperCase();
      _nineletterController.value = _nineletterController.value.copyWith(
        text: text,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('9 letter words',
            style: TextStyle(
              fontSize: 20,
            )),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  child: TextField(
                    controller: _nineletterController,
