...
class SevenLetterQuiz extends StatefulWidget {
  SevenLetterQuiz({super.key});
  @override
  State<SevenLetterQuiz> createState() => _SevenLetterQuizState();
}

class _SevenLetterQuizState extends State<SevenLetterQuiz> {
  int i = 0;
  final TextEditingController _sevenletterController = TextEditingController();
  ...
  void getWords() async {
    ...
    final x = await rootBundle.loadString("assets/csvfile/sevens.csv");
    setState(() {
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => SevenLetterQuiz()));
                  },
                ),
              ],
            );
  ...
  void checkWord() {
    if (_sevenletterController.text == '') {
      return;
    }
    ...
    setState(() {
      if (uniqueWords.contains(_sevenletterController.text.toLowerCase())) {
        if (!correctGuesses
            .contains(_sevenletterController.text.toUpperCase())) {
          correctGuesses.add(_sevenletterController.text.toUpperCase());
        }
        ...
        if (!incorrectGuesses
            .contains(_sevenletterController.text.toUpperCase())) {
          incorrectGuesses.add(_sevenletterController.text.toUpperCase());
        }
      }
    });

    _sevenletterController.clear();
  }
  ...
  void initState() {
    super.initState();
    getWords();
    _sevenletterController.addListener(() {
      final String text = _sevenletterController.text.toUpperCase();
      _sevenletterController.value = _sevenletterController.value.copyWith(
        text: text,
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('7 letter words',
            style: TextStyle(
              fontSize: 20,
            )),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  child: TextField(
                    controller: _sevenletterController,
