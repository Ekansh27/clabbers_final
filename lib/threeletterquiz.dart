...
class ThreeLetterQuiz extends StatefulWidget {
  ThreeLetterQuiz({super.key});
  @override
  State<ThreeLetterQuiz> createState() => _ThreeLetterQuizState();
}

class _ThreeLetterQuizState extends State<ThreeLetterQuiz> {
  int i = 0;
  final TextEditingController _threeletterController = TextEditingController();
  ...
  void getWords() async {
    ...
    final x = await rootBundle.loadString("assets/csvfile/threes.csv");
    setState(() {
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => ThreeLetterQuiz()));
                  },
                ),
              ],
            );
  ...
  void checkWord() {
    if (_threeletterController.text == '') {
      return;
    }
    ...
    setState(() {
      if (uniqueWords.contains(_threeletterController.text.toLowerCase())) {
        if (!correctGuesses
            .contains(_threeletterController.text.toUpperCase())) {
          correctGuesses.add(_threeletterController.text.toUpperCase());
        }
        ...
        if (!incorrectGuesses
            .contains(_threeletterController.text.toUpperCase())) {
          incorrectGuesses.add(_threeletterController.text.toUpperCase());
        }
      }
    });

    _threeletterController.clear();
  }
  ...
  void initState() {
    super.initState();
    getWords();
    _threeletterController.addListener(() {
      final String text = _threeletterController.text.toUpperCase();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('3 letter words',
            style: TextStyle(
              fontSize: 20,
            )),
            ...
            child: Row(
              children: [
                Expanded(
                  ...
                  child: TextField(
                    controller: _threeletterController,
